...
const Usage = () => {
        ...
        const fetchUsageData = async () => {
            try {
                ...
                if (dataResponse && dataResponse.data) {
                    setUsageData(dataResponse.data);
                    console.log(`✅ ${viewMode} usage data loaded:`, dataResponse.data);
                } else {
                    console.warn(`No usage data found for ${viewMode}`);
                    // Set fallback data
                    setUsageData(getFallbackData(viewMode));
                }
    ...
    const avgUsage = data.length > 0 ? (data.reduce((sum, d) => sum + d.usage, 0) / data.length).toFixed(0) : 0;
            ...
            {loading && (
                <div className="flex items-center justify-center min-h-96">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-stone-500">Loading usage data...</p>
                    </div>
                </div>
            )}
